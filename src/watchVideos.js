...
async function getLatestPostsFromAPI(accessToken, ownerId, count = 25) {
    try {
...
async function main() {
    try {
            ...
            try {
                browser = await puppeteer.launch({
                    ...
                    args: [
                        ...
                        config.proxy ? `--proxy-server=${config.proxy}` : null
                    ].filter(Boolean)
                });
        ...
        console.error(`[${formatTimestamp(new Date())}] Критическая ошибка:`, error);
    }
}
...
async function runWithRestart() {
    while (true) {
        console.log(`[${formatTimestamp(new Date())}] Запуск новой итерации скрипта...`);
        await main(); // Запускаем основную функцию
        console.log(`[${formatTimestamp(new Date())}] Итерация завершена. Перезапуск через 60 секунд...`);
        await new Promise(resolve => setTimeout(resolve, 3600000)); // Задержка перед перезапуском (60 секунд)
    }
}
